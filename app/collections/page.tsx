    ...
    <AppShell>
        ...
        {(["all", "complete", "in_progress", "not_started"] as const).map((s) => (
          <button
            ...
            className={`text-sm px-3.5 py-1.5 rounded-full border transition ${
              statusFilter === s ? "bg-violet text-white border-violet" : "border-line text-inkmuted hover:border-violet"
            }`}
          >
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <input
          ...
          className="flex-1 min-w-[200px] border border-line rounded-lg px-3 py-2 text-sm bg-surface text-ink placeholder-inkmuted focus:border-violet"
        />
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value as TypeFilter)}
          className="border border-line rounded-lg px-3 py-2 text-sm bg-surface text-ink placeholder-inkmuted bg-surface text-ink focus:border-violet"
        >
